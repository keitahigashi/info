...
### 【完全保存版】Claude Code 2026年6月アップデート全まとめ：Artifacts・Design刷新・Fable 5統合（チャエン）（自動収集 2026-06-24）
2026年6月21日公開（note/チャエン）。6月の全主要アップデートを網羅：6/8〜12の開発者向け新機能ラッシュ（/cd・サブエージェント入れ子・--safe-mode・fallbackModel）、6/9のFable 5統合、6/12〜13の輸出規制による全世界停止、6/15の対話vs自動化課金分離、6/17のClaude Design刷新（/design-sync追加）、6/18のArtifacts in Claude Codeベータ開始。総評：「コーディングツールからチームの仕事そのものを動かすプラットフォームへ進化」。Artifactsにより作業成果が同URLで自動更新される共有Webページとして共有可能になった点が特に注目。
> 詳細: references/reference_chaen_june2026_updates_all.md を参照

### 「AIが自らを作る」と予言した8日後、Claude Fable 5は消えた（生成AIストリーム・窓の杜）（自動収集 2026-06-24）
2026年6月19日公開（窓の杜 生成AIストリーム）。6月5日発表の論文「When AI builds itself」でAnthropicが「エンジニアのコード生成量が2024年比8倍、新規コードの80%超がClaudeによる」と報告した8日後、米国輸出管理指令により全世界でFable 5が停止した経緯を分析。著者の教訓：①「強い道具は借り物」——依存した道具が突然消えるリスク②地政学リスクはAI利用の想定外タイミングで現実化③ソブリンAI・ローカルLLMの戦略的価値向上。実務対応としてfallbackModel設定と、コアロジックのAI非依存設計維持を推奨。
> 詳細: references/reference_aistream_fable5_disappeared_june2026.md を参照
