...
### Claude Code Artifacts はTeam/Enterprise限定——個人Maxプランでは利用不可（lnest_knowledge/danielvo）（自動収集 2026-06-23）
2026年6月19日公開（Zenn/lnest_knowledge）。6月18日発表のClaude Code Artifactsを実機検証。従来の「Claude.ai Artifacts（HTMLプレビュー）」とは別機能で、Claude Codeの成果物を組織内共有ライブWebページとして公開する機能。プラン制限：Team/Enterpriseのみ利用可能、個人Max不可。技術制約：単一HTMLのみ・外部API不可・16MiB以下・組織内限定。活用用途：長い作業ログのダッシュボード化、ルーチン出力の可視化、進捗共有ページ。「個人開発者には縁遠い機能だが、Teamプラン以上の組織では有効」という結論。
> 詳細: references/reference_claude_code_artifacts_verification_lnest.md を参照

### Claude Designが/design-syncコマンドでClaude Codeと双方向連携——9プラットフォームコネクタも追加（Uravation/佐藤傑）（自動収集 2026-06-23）
2026年6月20日公開（Uravation）。6月17〜18日にリリースされたClaude Designの大型アップデート速報。①`/design-sync`・`/design`コマンドでClaude Codeからデザインを直接作成・同期②デザインシステム取込機能の強化と自動検証機能搭載③PDF・PowerPointエクスポート対応④9プラットフォームコネクタ追加（Webflow/WordPress/Shopify/Figma/Notion/Adobe/Canva等）⑤admin役割でブランドカラー・フォントを組織一元管理。使用枠もClaude Design・Code・通常チャットが統一プールに変更され「Designを使うとチャット枠が食われる」問題が解消。
> 詳細: references/reference_claude_design_code_sync_uravation_june2026.md を参照
