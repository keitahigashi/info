...
### MCPサーバーおすすめ2026 Claude Code・Cursor別10選（自動収集 2026-04-27）
12,000本超から「メンテ頻度・ツール定義数・公式性」3基準で厳選した10選。鉄板5選: ①Context7（ドキュメント注入・API最新化）②GitHub MCP（51ツール・PR管理）③Filesystem MCP（Anthropic公式）④Playwright MCP（ブラウザ自動化）⑤Brave Search MCP（月2,000クエリ無料）。用途別5選: ⑥Supabase（DB・スキーマ）⑦Notion（ドキュメント）⑧Slack（チャット自動化）⑨Firecrawl（Web調査）⑩Serena（LSP統合・コード理解）。複数MCP同時実行時はプロンプトインジェクションリスクと干渉に注意。
> 詳細: references/reference_mcp_servers_10_ai_career_japan.md を参照

### Adobe for creativityコネクター：50+Creative Cloudツールを自然言語で操作（自動収集 2026-04-30）
2026年4月28日、Adobe公式のClaudeコネクター発表。Photoshop・Illustrator・Firefly・Premiere・InDesign等50以上のツールを自然言語で統合オーケストレーション可能。セットアップ: Web版Claudeの「Connectors」からAdobe for creativityをインストールするのみ。全プランでAdobe IDなしで基本利用可能（拡張機能は有料プラン向け）。活用例: LPキービジュアル生成・SNSバナー量産・YouTube動画のマルチプラットフォーム変換。注意: 著作権確認が必須、ビジュアル評価能力の重要性が増大。
> 詳細: references/reference_adobe_claude_connector_shiftb.md を参照

### Claude Code MCPサーバー設定完全ガイド：推奨5選と実装例（自動収集 2026-04-27）
「まずfilesystemとgitの2本から始める」入門推奨ガイド。推奨5選: filesystem（ローカルファイル操作）・git（リポジトリ操作）・github（PR/Issue管理）・playwright（ブラウザ自動化）・postgres（DB操作）。`~/.claude.json`の`mcpServers`セクションに追加する方式でmacOS/Linux/Windows WSL環境別に手順を詳述。カスタムサーバー開発はTypeScript/PythonのSDKで対応。セキュリティ: APIキーは環境変数または`.env.local`管理・`.gitignore`登録が必須。
> 詳細: references/reference_claude_code_mcp_server_guide_ainow.md を参照

### Claude 9コネクタ完全解説：Blender・Adobe CC・Fusion・Abletonを自然言語で操作（自動収集 2026-05-02）
MCPベースの公式コネクタ群（全プラン・追加コストなし）。9コネクタ: ①Adobe for Creativity（50+CC製品）②Blender（3D・Python生成）③Autodesk Fusion（CAD設計）④Ableton（操作ガイド）⑤Splice（音源ライブラリ検索）⑥Affinity by Canva（デザイン自動化）⑦SketchUp（建築3D）⑧Resolume Arena/Wire（VJ映像）。セットアップ: claude.ai設定の「Connectors」から5分でインストール完了。会話ごとの有効化設計（常時接続ではない）。企業利用注意: セキュリティポリシー確認必須・AI出力のレビュー担保・段階的導入推奨。
> 詳細: references/reference_claude_connectors_9_uravation.md を参照

### Plugin Marketplace社内配布完全ガイド：3リポジトリ超えがプラグイン化の潮時・marketplace.json/plugin.json設計（自動収集 2026-05-10）
同じSkillを3リポジトリ以上にコピーした時がPlugin Marketplace化の判断基準。2層構造: marketplace.json（カタログ）＋plugin.json（各プラグイン）。最小構成はSKILL.md＋plugin.json＋marketplace.jsonの3ファイル。5種のソース指定（相対パス・github・url・git-subdir・npm）でPrivateリポジトリも`gh auth login`済みなら追加設定不要。Claude Cowork（企業向け）は組織コンソールから社員に一斉配布可能。運用原則: PRレビュー必須・version明示・デプロイ系Skillには本番事故対策を最初から組み込む。
> 詳細: references/reference_claude_code_plugin_marketplace_internal_hatenabase.md を参照

### Claude for Microsoft 365 一般提供開始：Excel・PowerPoint・Word間でコンテキスト共有、Outlookベータ公開（自動収集 2026-05-10）
2026年5月8日、Claude for Microsoft 365（Excel・PowerPoint・Word）が全有料プランで一般提供開始（Microsoftマーケットプレースから無償DL・有料プラン必要）。最大の特徴はアプリ間コンテキスト共有：Excelで集計したデータがPowerPointやWordにもそのまま引き継がれ、一方のアプリでの操作が他アプリにも反映される。Claude Outlookはベータ版で受信トレイ分類・メール返信下書き・カレンダー連携が可能。搭載モデルはClaude Opus 4.7。
> 詳細: references/reference_claude_m365_excel_ppt_word_madonomori.md を参照

### Claude Platform on AWS 正式リリース：Anthropicがデータ処理・AWSは認証・請求のみ担当（自動収集 2026-05-12）
2026年5月11日正式リリース。データ処理の分離が最大の特徴で、顧客データはAWS境界外でAnthropicが処理し、AWSは認証（IAM/APIキー）・課金（Marketplace経由）・監査ログのインフラのみを担当。Claude APIの全機能を初日から提供、トークン価格はClaude APIと同一。対応モデル: Opus 4.7・Sonnet 4.6・Haiku 4.5。東京リージョンを含むグローバル主要リージョン対応。Amazon Bedrockとの違い: データレジデンシー要件が厳格な金融・医療向け、Anthropic管理UIを利用する用途に適している。
> 詳細: references/reference_claude_platform_on_aws_crenavi.md を参照

### AnthropicがStainlessを買収：モデル・MCP標準・SDKツールチェーンを完全掌握（自動収集 2026-05-19）
2026年5月18日（米国時間）、AnthropicがSDK自動生成・MCPサーバーツール企業Stainlessを買収（3億ドル超）。Stainlessは2022年設立でAnthropicの全公式SDK（TypeScript/Python/Go/Java/Kotlin）を担当し、OpenAI・Googleも同ツールを利用していた。買収後は競合他社の新規利用不可に。この買収でAnthropicはモデル（Claude）＋接続標準（MCP）＋SDKツールチェーンの三要素を一手に掌握。MCPは2024年11月公開以来、月間ダウンロード9700万回・公開サーバー1万超に成長。既存顧客の生成済みSDKは完全保持可能。
> 詳細: references/reference_anthropic_stainless_itmedia.md / references/reference_anthropic_stainless_gigazine.md を参照

### Claude CodeのMCPサーバー完全解説：ローカル/リモート型・企業公式・Playwright活用例（自動収集 2026-05-25）
マイナビニュース解説。MCPサーバーはClaude Codeに「目と手足」を与えるオープン規格。種類は①ローカル型（PC上動作）②リモート型（クラウド）に大別され、提供元は企業公式（Microsoft・GitHub・Slack・Figma等）／コミュニティ製／自作の3分類。利点はコンテキスト消費が少なく大型データ処理に優れる点。Microsoft提供のPlaywright MCPでは自然言語のみでブラウザ操作（フォーム入力・スクリーンショット付きテスト報告書の自動生成）が実現。AWS MCPは5月にGA（一般提供）を開始し、IAMガードレール・CloudWatchメトリクス・CloudTrailロギングを伴った安全なAWSアクセスが可能に。
> 詳細: references/reference_claude_code_mcp_server_mynavi_0525.md を参照

### Claude Code・MCP・Skillsを組み合わせた業務特化型AIエージェント開発が商用展開フェーズへ（自動収集 2026-06-03）
2026年6月3日、株式会社UravationがClaude Code・MCP・Skillsを組み合わせた「業務特化型AIエージェント開発」サービスを強化発表（PR TIMES）。「汎用AIの導入だけでは現場が動かない」という課題に対応し、各社の業務フローに合わせてエージェントを設計・開発。想定活用領域：営業（提案資料作成・顧客リサーチ）・経理（定型書類・データ整理）・カスタマーサポート（FAQ回答案）・リサーチ企画（情報収集・要約）。要件ヒアリングから研修・伴走まで一貫提供。Claude Codeのエコシステム（MCP・Skills・Hooks）を組み合わせたベンダーソリューションが商用化フェーズに入ったことを示す業界動向。
> 詳細: references/reference_uravation_mcp_agent_service_prtimes.md を参照

### homulaが「agens」提供開始：セルフホスト×Multi-LLM×組織統制を両立するエンタープライズ向けAIエージェント実行基盤（自動収集 2026-06-04）
2026年6月2日、株式会社homulaがAIエージェント実行基盤「agens（エージェンス）」をセルフホスト・オンプレミス構成で提供開始。3大特徴：①データを社外に出さない設計（金融・医療向けのセキュリティ要件に対応）②Multi-LLM対応（Claude/GPT/Gemini等ベンダー非依存）③組織統制機能（権限管理・監査ログ）。構成要素はClaude Cowork/Codeの設計を参考に：チャット・スキル・サンドボックス実行・フォルダ・MCP連携を統合。中〜大企業向け先行導入プログラムも同時開始。SaaS型AIを導入できない規制業種にとって、Claude Codeのエージェント体験をオンプレ・プライベートクラウドで再現する選択肢として注目。
> 詳細: references/reference_homula_agens_release_prtimes.md を参照

### Claude Code + GitHub MCPでZenn自動公開→実績管理パイプラインを構築（muramasa0228）（自動収集 2026-06-21）
2026年6月20日公開（Zenn/muramasa0228）。「push = 公開」かつURLが事前確定するZennを自動化媒体として選択（noteはpushで公開不可・QiitaはURL事前確定不可）。リポジトリ構成：`articles/`（Zenn記事）＋`career/index.json`（実績管理）を同一リポジトリに統合し、JSONからREADMEの実績一覧を自動生成。GitHub MCP設定のポイント：fine-grainedトークンに`contents:write`・`pull_requests:write`を付与。設計思想：「発信媒体は自動化の可能性で選定する」 — Claude Codeがコンテンツ生成・GitHubへのpush・実績JSON更新を一括担当。
> 詳細: references/reference_claude_code_mcp_zenn_pipeline_muramasa.md を参照

### Claude Codeブラウザ操作4ツール徹底比較：Claude in Chrome・chrome-devtools-mcp・agent-browser・Playwright MCP（takna）（自動収集 2026-06-24）
2026年6月21日公開（Zenn/takna）。Claude Code環境でのブラウザ自動化に使える4ツールを用途別に評価。推奨：実ログイン操作→Claude in Chrome（ブラウザセッションをそのまま利用）、パフォーマンス診断→chrome-devtools-mcp（DevTools API直接アクセス）、CLI効率重視→agent-browser（Vercel Labs製、軽量高速）、クロスブラウザE2Eテスト→Playwright MCP（Microsoft製、CI/CD統合向き）。chrome-devtools-mcpは2026年6月にMCPモード追加直後で実運用事例は少ない段階。現実的な運用は複数ツール組み合わせが有効。
> 詳細: references/reference_browser_tools_comparison_takna.md を参照

### Claude Design刷新：/design-syncと/designコマンドでClaude Code双方向連携が本格化（Uravation）（自動収集 2026-06-24）
2026年6月20日公開（Uravation）。6月17日実施のClaude Design大幅刷新の全容：①`/design-sync`コマンドでデザイン↔コードを双方向同期（handoff問題を解決）②`/design`でClaude CodeからDesignを直接呼び出し③GitHub・デザインファイル・アップロードからデザインシステム取込強化④PDF・PowerPoint出力とAdobe/Canva/Miro等9プラットフォームコネクタ追加⑤Admin役割でデザインシステムをロック（組織統制）⑥使用枠をDesign・Code・チャット間で統合。Claude Designは「Figmaの代替」ではなく「Claude上でエンジニアとデザイナーが協業するデザイン制作ハブ」として進化。
> 詳細: references/reference_claude_design_june2026_uravation_codesync.md を参照

### Claude Designデスクトップアプリ対応ハンズオン：「AI臭さが消えた本格デザイン」が生成される（91works）（自動収集 2026-06-24）
2026年6月19日公開（Zenn/91works Tech Blog）。Claude DesignのClaudeデスクトップアプリ対応を受け2機能を実検証。①Make something new：提案スライド・ダッシュボードリデザイン・Webサイト全面リデザインの3ケースで検証、特にWebリデザインでは「AI的な見た目が消えた本格的なデザイン」と高評価。②Design systems：コードベースから自動デザインドキュメント生成（「Create here」vs「Create using Claude Code」の2アプローチ）。トークン消費は1セッション2〜6%程度と軽微。位置付けは「Figmaの代替ではなくClaudeとデザインツールの間を埋める存在」。
> 詳細: references/reference_claude_design_handson_91works.md を参照
