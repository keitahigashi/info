...
### Claude Code × Obsidian Vault プロジェクト設計（自動収集 2026-04-20）
Obsidian Vaultに成果物を集約するClaude Code専用プロジェクト。CLAUDE.mdにファイル配置ルール表を記載しClaudeが自動分類。settings.jsonで`model: opus-4-7`・`effortLevel: xhigh`・`CLAUDE_CODE_DISABLE_ADAPTIVE_THINKING: 1`を固定。.mcp.jsonでAWS Documentation MCP常時有効化。ポータビリティ設計: 絶対パス禁止・`/`統一・UTF-8 BOMなし・LF改行。
> 詳細: references/reference_claude_code_obsidian_vault_project.md を参照

### Claude Code 7層設定の本番運用ガイド（自動収集 2026-04-22）
...
### Claude Teamプラン管理画面完全ガイドとBedrock移行事例（自動収集 2026-04-25）
カンリー実運用経験によるTeamプラン管理画面全解説: Standard $25/人・Premium $125/人の二層料金構造（2026年1月値下げ）、組織全体/シート種別/個人別の3段階使用量制御、Bedrockからの3段階移行（2025年6月→2025年末→2026年1月全員移行）。セキュリティ設定: 招待リンクOFF推奨・バイパス権限OFFを明示。注意: 新機能がサイレントにデフォルトブロック状態で追加されるため定期確認が必要。セッション単位の詳細分析にはOpenTelemetry連携が必須。
> 詳細: references/reference_claude_team_plan_guide_canly.md を参照

### Claude Code 6月15日改定 — プログラム的利用が月次クレジット制へ移行（自動収集 2026-05-17）
2026年6月15日よりAgent SDK・`claude -p`・GitHub Actions等のプログラム的利用が月次クレジット体系に分離。対話的利用（ターミナル・IDE）は従来通りサブスクリプション内に据え置き。プランごとに月次クレジット量が変わる（Pro=$20が最小〜Max 20x=$200が大容量）。影響3段階: ライトユーザー=ほぼ影響なし、中規模=使い方設計見直し、ヘビー=予算管理へ移行。対応: ①利用方法の棚卸し（対話的 vs プログラム的）②タスク分類・優先順位付け③チームでのルール化④コスト監視整備。
> 詳細: references/reference_claude_code_pricing_shift_june2026.md を参照

### Claude Code 料金完全ガイド【2026年6月版】：TCO試算・失敗パターン4選（Uravation）（自動収集 2026-06-11）
Uravationが100社以上の導入実績をもとにまとめた2026年6月版料金完全ガイド。6プランの体感差：Free（試用）→Pro $20（1日1〜2時間）→Max 5x $100（4時間以上）→Max 20x $200（並列運用）→Team $25/席（法人一元化）→Enterprise（カスタム）。TCO試算3パターン：個人軽量$49〜・個人ヘビー$150〜・法人3名$300〜$405。失敗パターン4選：①無料続行でコンテキスト不足②パターン分析なしでMax 20x即選択③個人クレカ経費清算の非効率④為替変動見積もり欠落。推奨移行パス：Free→Pro→Max 5x→Max 20xまたはTeam。
> 詳細: references/reference_claude_code_pricing_complete_uravation_june2026.md を参照

### Claude Code 6月15日改定・実務運用——「2つのおさいふ」戦略（DevelopersIO・classmethod）（自動収集 2026-06-15）
利用枠が「おさいふ1（インタラクティブ）」と「おさいふ2（プログラム的）」に分割。Remote Routine（/schedule）はおさいふ1を消費するため夜間バッチの第一候補。重要注意点：APIキーを環境変数で指定するとサブスク枠外の従量課金になる。GitHub Actionsは認証方式によって消費先が異なる（APIキー→従量課金、OAuth token→おさいふ2）。overflow toggleデフォルトOFFでクレジット枯渇後は自動停止——意図しない高額請求を防ぐ。ClaudeとCodexをコスト効率よく組み合わせる戦略も有効。
> 詳細: references/reference_claude_pricing_reform_classmethod_june2026.md を参照
