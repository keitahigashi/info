...
| ファイル | 内容 |
|---------|------|
| [**00-quickstart.md**](00-quickstart.md) | **はじめてガイド（エンジニア向け）** |
| [**00-quickstart-nontechnical.md**](00-quickstart-nontechnical.md) | **はじめてガイド（非エンジニア・企画職向け）** |
...
| [12-loop-engineering.md](12-loop-engineering.md) | ループエンジニアリング（自律ループの設計と検証） |
...
| 要素 | 保存先 | 永続性 | 役割 |
|------|--------|--------|------|
| **CLAUDE.md** | リポジトリ内 | Git管理 | ルール・ファイル構成をセッション開始時に自動読み込み |
| **SKILL.md** | リポジトリ内 | Git管理 | 記事収集ワークフローの詳細手順 |
| **REFERENCES.md** | リポジトリ内 | Git管理 | 収集済み記事の一覧（重複チェックの単一ソース） |
| **references/*.md** | リポジトリ内 | Git管理 | 各記事の詳細情報（要約・コード例・設定例等） |
| **MEMORY.md** | Claudeメモリ | セッション間で永続 | アーキテクチャ変更メモ・ユーザー方針 |
