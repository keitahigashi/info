...
### NotionのClaude全モデル一時無効化事例：AI多プロバイダー戦略の可用性確保における実証（自動収集 2026-06-08）
...
### Fable 5停止後の実務対応ガイド——「AIは止まりうるインフラ」の設計思想（Fyve）（自動収集 2026-06-15）
Fable 5は2026年6月12日に米商務省輸出管理指令で停止（アカウント凍結ではなく他モデルは継続）。即時対応：環境変数 `ANTHROPIC_MODEL=claude-opus-4-8` または `settings.json` の `model:` フィールドで切り替え。補償：レート制限リセットの報告があるがAnthropicは「補償」と公式には明言せず。設計思想の転換：「AIは電力・通信と同様に突然止まりうるインフラ」として設計すべき——複数モデル運用・モデルIDの環境変数一元管理・フォールバック機構の事前設計が必須。復旧は2026年6月15日時点でも確定スケジュールなし。
> 詳細: references/reference_claude_fable5_suspension_guide_fyve.md を参照
