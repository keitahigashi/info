...
### availableModels強制回避の抜け穴修正・Hook条件パスマッチ改善（Claude Code v2.1.176 / Qiita・picnic）（自動収集 2026-06-16）
2026年6月13日リリースの重要セキュリティ修正。環境変数経由でのブロック済みモデルへのリダイレクト経路を遮断・`/fast`コマンドによる許可リスト外モデル切替えも拒否。Hookの`Edit(src/**)`・`Read(.env)`パスパターンが正しくマッチするよう修正——意図通りにフィルタリングが機能しなかった問題が解消。Bedrock認証情報キャッシュを固定1時間から有効期限ベースに変更して認証エラー減少。
> 詳細: references/reference_claude_code_v21176_picnic.md を参照
