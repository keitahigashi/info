...
- [プロンプトの書き方](#プロンプトの書き方)
- [コンテキスト管理](#コンテキスト管理)
- [サブエージェント活用のコツ](#サブエージェント活用のコツ)
- [メモリシステムの活用](#メモリシステムの活用)
- [セッション最適化チェックリスト](#セッション最適化チェックリスト)
- [Anthropic社員の実践テクニック](#anthropic社員の実践テクニック)
- [フィードバックループの4段階](#フィードバックループの4段階)
- [Rate Limit との付き合い方](#rate-limit-との付き合い方)
- [仕様駆動開発（SDD）](#仕様駆動開発sdd)
- [Why/Whether を手放すな](#whywhether-を手放すな)
- [プチ仕様駆動開発](#プチ仕様駆動開発)
- [MCP vs Skills vs CLAUDE.md の使い分け](#mcp-vs-skills-vs-claudemd-の使い分け)
- [外部エージェント集の活用（agency-agents）](#外部エージェント集の活用agency-agents)
- [実践メモ](#実践メモ)
...
### Claude Code 全機能8モジュール34機能体系：企業ROI13倍実績（自動収集 2026-04-29）
...
### Claude自動化利用の課金分離（2026年6月15日施行）：月次クレジット制の全容と注意点（sakamoto66）（自動収集 2026-06-23）
2026年5月14日公開（Qiita/@sakamoto66）。6月15日施行の「対話vs自動化」課金分離を事前解説。`claude -p`・Agent SDK・GitHub Actionsなどの自動化利用がサブスク枠から分離され、専用の「月次クレジット（USD）」に移行。クレジット額：Pro $20/月、Max 5x $100/月、Max 20x $200/月。メリット：「自動化で対話枠が食われる」問題が解消。落とし穴4点：①クレジット繰り越し不可②メールからのopt-in必須③Extra usage設定が必要④モデルによる消費速度差（Opus>Sonnet>Haiku）。なお6月中旬にAnthropicが一部変更の見送りをアナウンスしており、最新状況は公式ドキュメント要確認。
> 詳細: references/reference_claude_billing_automation_sakamoto66.md を参照
