...
### Dynamic Workflows 6パターン完全ガイド（Zenn・aria3）：3コアAPIと実践パターンの体系化（自動収集 2026-06-08）
Dynamic Workflowsの本質は「Claudeがタスク専用ハーネスをその場で書く仕組み」。単一コンテキスト問題（エージェント的怠慢・自己優先バイアス・目標ドリフト）を構造的に解決する。3コアAPI：`agent()`（単一起動）・`parallel()`（並列収集）・`pipeline()`（チェーン変換）。6パターン：①分類して実行②展開して統合（Map-Reduce）③敵対的検証④生成してフィルタリング⑤トーナメント⑥完了まで繰り返す。運用ポイント：`/goal`コマンドで目標ドリフト防止・`/loop`で定期実行自動化・明示的トークン予算でコスト暴走防止。現在Max・Team・Enterpriseのリサーチプレビュー段階。
> 詳細: references/reference_claude_code_dw_6patterns_aria3.md を参照

### Dynamic Workflows ハンズオン（ディアシステム Tech Blog）：JavaScriptオーケストレーション自動生成とBun→Rust 75万行移植事例（自動収集 2026-06-08）
