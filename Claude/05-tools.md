...
### Claude Code MCP × 広告運用監視 — Meta/Googleトークン期限切れを自動検知（自動収集 2026-05-17）
F2T Blogの実装事例。Meta広告アクセストークンは約60日で期限切れとなり夜間にサイレント停止するため、Claude Code + MCPサーバーで20個のMCPサーバー健全性を毎朝確認する自動監視システムを構築。Metaトークンの自動更新ループ（残り14日以下で新トークンに自動切替）、launchd/cronでスケジューリング、Slack webhook異常通知を組み合わせ、月額$20（Claude Pro）で実現。代理店外注（月5〜10万円）やSupermetrics（月$99〜$499）と比べて大幅コスト削減。コピペ可能なbashスクリプトとlaunchd設定ファイルを公開。対応プラットフォーム: Google Ads・Meta Ads・TikTok Ads。
> 詳細: references/reference_claude_code_mcp_ads_monitoring_f2t.md を参照

### Fable 5 API実装ガイド：refusalはHTTP 200・server-side fallbackはbeta APIで実装（Zenn・yutabeee）（自動収集 2026-06-12）
...
### /loopとRoutinesの使い分け：7日制限・クラウド実行・トリガー種別（Claude Code研究所）（自動収集 2026-06-21）
2026年6月15日公開（note/Claude Code研究所）。/loopはセッション内の反復実行（`/loop <間隔> <プロンプト>`、自然言語間隔指定可）、**重要制限：作成から7日後に自動削除**。Routinesはクラウド環境で動作しPC電源OFFでも継続、スケジュール・API・GitHubイベントの3種トリガーに対応。使い分け指針：7日以内の短期反復→/loop、週次・月次・継続的自動化→Routines、外部イベント起動→Routines。実運用ベストプラクティス：①プロンプトに明示的なトークン予算を設定（「最大Nターンで完了」）②エラー時の通知設定を必ず入れる③/loopの7日制限に気づかず停止するトラブルに注意。
> 詳細: references/reference_claude_code_autopilot_loop_routines_claudelab.md を参照
